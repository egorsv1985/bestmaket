...
?>
				...
				<div class="form-green form-calculator ajax-form" id="smallForm_<?=$FORM_ID;?>">
...
<?
$APPLICATION->IncludeFile("/includes/text-form-calculator.php", array(), array(
	'NAME' => 'текст',
	'MODE' => 'html'
));
?>
					</div>
					<form name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data" data-id="calculatorForm">
...
?>
						<div class="form-row<?=($propertyID == 'NAME' ? ' clear' : '');?>">
<?if ($INPUT_TYPE != 'F'):?>
							<label>
<?endif;?>
<?
...
<?		
	if ($propertyID == 22):
  	$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"] = 'Размер макета';	
?>
	<div class="row-col">
<?endif;?>
								<span class="row-name">
...
<?if ($propertyID == 22):?>
	<div class="col-contaner"> 
		<div class="col-name">Длинна</div>
<?endif;?>
...
?>
	...
	<div class="col-contaner"> 
		<div class="col-name">Ширина</div>
		<input type="text" name="PROPERTY[23][<?=$i?>]" value="<?=$value?>" class="inp-text" />
	</div>
	<div class="clear"></div>
</div>
<?endif;?>
...
<?if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"])):?> <span class="starrequired">*</span><?endif?>
								...
								foreach ($arResult["PROPERTY_LIST_FULL"][$propertyID]["ENUM"] as $key => $arEnum) {
									...
									if ($arParams["ID"] > 0 || count($arResult["ERRORS"]) > 0)
									{
										...
										{
											if ($key == $arElEnum["VALUE"]) {$checked = true; $value = $arEnum["VALUE"]; $valueID = $arEnum["ID"]; break;}
										}
									}
									else
									{
										if ($arEnum["DEF"] == "Y") $checked = true; $value = $arEnum["VALUE"]; $valueID = $arEnum["ID"];
									}
								}
								?>
								...
								<div class="pseudo-select">
									<div class="select inp-text"><?=$value;?></div>
									<div class="options">
										<?foreach ($arResult["PROPERTY_LIST_FULL"][$propertyID]["ENUM"] as $key => $arEnum):?>
										<div value="<?=$key?>"><?=$arEnum["VALUE"]?></div>
										<?endforeach;?>
									</div>
									<input type="text" class="hidden" name="PROPERTY[<?=$propertyID?>]" value="<?=$valueID;?>">
								</div>
...
<?endif;?>
	...
	<?if (strlen($arParams["LIST_URL"]) > 0):?><a href="<?=$arParams["LIST_URL"]?>"><?=GetMessage("IBLOCK_FORM_BACK")?></a><?endif?>
						...
						<div class="form-submit">
							<button type="submit" name="iblock_submit" value="1">узнать стоимость</button>
						</div>
					</form>
			  </div>