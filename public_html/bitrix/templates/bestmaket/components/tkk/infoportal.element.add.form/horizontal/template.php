...
if (!isset($FORM_ID))
	...
	$FORM_ID++; 
$arResult['ELEMENT']['NAME'] = 'Заявка';
?>
<!--
<?
print_r($arResult['ELEMENT']['NAME']);
?>
-->

				<div class="ajax-form" id="smallForm_<?=$FORM_ID;?>">
	...
	<?if ($arParams["MAX_FILE_SIZE"] > 0):?><input type="hidden" name="MAX_FILE_SIZE" value="<?=$arParams["MAX_FILE_SIZE"]?>" /><?endif?>
		...
		<?if (is_array($arResult["PROPERTY_LIST"]) && count($arResult["PROPERTY_LIST"]) > 0):?>
			...
			<?foreach ($arResult["PROPERTY_LIST"] as $propertyID):?>
						<div class="form-row <?=$propertyID;?> <?=($propertyID == 'NAME' ? 'hidden' : '');?>">
							<div class="label">
...
<?if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"])) { $PLACEHOLDER .= ' *'; }?>
								...
								<textarea class="inp-text" rows="1" placeholder="<?=$PLACEHOLDER;?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea>
								<?
								}
							break;
...
<?endif?>
<?if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"])) { $PLACEHOLDER .= ' *'; }?>
								<input type="text" placeholder="<?=$PLACEHOLDER;?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" size="25" value="<?=$value?>" class="inp-text" /><?
		...
		<?endif?>
						<div class="form-submit">
							<button type="submit" name="iblock_submit" value="1">Узнайте цену сейчас</button>
						</div>
