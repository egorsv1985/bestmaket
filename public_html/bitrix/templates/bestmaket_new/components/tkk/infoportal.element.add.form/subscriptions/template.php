...
//exit();
?>
<div class="subscriptions ajax-form" id="<?=$arResult["formID"];?>">
...
<?if (strlen($arResult["MESSAGE"]) > 0):?>
	<div class="alert">
		<div class="alert-contaner">
			<div class="top">
				<div class="title"><span>Подписка успешно оформлена</span></div>
			</div>
			<!--
...
<?endif?>
	...
	<div class="title">Подпишитесь на наш блог</div>
	<div class="content">
		Подписка позволит Вам получать много интересных материалов о создании макетов
	</div>
	
	<form name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data" data-id="subscriptionsForm">
	...
	<?if ($arParams["MAX_FILE_SIZE"] > 0):?><input type="hidden" name="MAX_FILE_SIZE" value="<?=$arParams["MAX_FILE_SIZE"]?>" /><?endif?>
			...
			<?foreach ($arResult["PROPERTY_LIST"] as $propertyID):?>
						...
						<textarea cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea>
							...
							case "N":
								...
								?>
								<input type="text" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" value="<?=$value?>" placeholder="<?if (intval($propertyID) > 0):?><?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?><?else:?><?=!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)?><?endif?>" <?=(in_array($propertyID, $arResult["PROPERTY_REQUIRED"]) ? ' class="required" required="required"' : '')?>/><?
		...
		<div class="form-submit">
			<button type="submit" name="iblock_submit" value="<?=GetMessage("IBLOCK_FORM_SUBMIT")?>">Подписаться</button>
		</div>
